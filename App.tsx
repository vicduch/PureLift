...
import { getExercises, getSets, getRoutines, saveSets, saveExercise, saveRoutines, getActiveRoutineId, setActiveRoutineId } from './store';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setLocalActiveRoutineId(getActiveRoutineId());
  }, []);
  ...
  const finishWorkout = () => {
    ...
    sessionExerciseIds.forEach(exId => {
      ...
      if (newWeight !== ex.referenceWeight) {
        ...
        saveExercise(updatedEx);
      }
    });

    saveSets(currentSessionSets.filter(s => s.completed));
    setAllSets(getSets());
    setExercises(getExercises());
                ...
                <div className="mt-12 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {exercises.map(ex => {
                      ...
                      return (
                        <div key={ex.id} className={`p-4 rounded-2xl border-2 flex items-center justify-between transition-all ${isSelected ? 'border-blue-500 bg-blue-50/50' : 'border-white bg-white hover:border-slate-100'}`}>
                          <div>
